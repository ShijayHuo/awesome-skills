...
- [/code-implement](./skills/code-implement/SKILL.md) - The default entry point for implementation work. A self-contained orchestrator skill that implements code changes through a direct-by-default workflow with explicit Planning, Implementation, Refactor, and Done stages. It inlines a minimal TDD core and Clean Code core instead of handing off to `/tdd` or `/clean-code`, escalating to heavier process only when behavior and risk justify it.
- [/key-decision-vetting](./skills/key-decision-vetting/SKILL.md) - A finite, high-signal design review that pressure-tests the most consequential decisions, sharpens domain terminology, and syncs the results into `CONTEXT.md` and architecture decision records (ADRs). This skill is adapted from `/grill-with-docs` for bounded decision vetting.
- [/grill-with-docs](./skills/grill-with-docs/SKILL.md) - An exhaustive, branch-by-branch design grilling session for aligning requirements, breaking down features, unifying domain terms and concepts, and syncing the results into `CONTEXT.md` and architecture decision records (ADRs). If you only need a finite review of the highest-impact decisions, prefer [/key-decision-vetting](./skills/key-decision-vetting/SKILL.md). This skill comes entirely from [Mattpocock: Grill with doc](https://github.com/mattpocock/skills/blob/main/skills/engineering/grill-with-docs/SKILL.md).
- [/tdd](./skills/tdd/SKILL.md) - Test-driven development. Standalone reference for pure TDD sessions; `/code-implement` already inlines a minimal TDD core for everyday implementation. This skill comes entirely from [Mattpocock: TDD](https://github.com/mattpocock/skills/blob/main/skills/engineering/tdd/SKILL.md).
...
- [/clean-code](./skills/clean-code/SKILL.md) - Clean up code based on the principles of *Clean Code*. Standalone reference for dedicated cleanup; `/code-implement` already inlines a minimal Clean Code core for everyday implementation. This skill comes entirely from this repository's local skill document, currently imported from [antigravity-awesome-skills](https://github.com/sickn33/antigravity-awesome-skills/blob/main/skills/clean-code/SKILL.md).
