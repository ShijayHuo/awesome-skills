...
## skills 列表
### 必备
- [/code-implement](./skills/code-implement/SKILL.md) —— 实现类工作的默认入口。一个自包含编排型 skill，以“默认直接实现”的工作流完成代码变更，包含明确的 Planning、Implementation、Refactor、Done 四个阶段。它把最小化的 TDD 核心与 Clean Code 核心内嵌在自身，而不是跳转交给 `/tdd` 或 `/clean-code`，只在行为与风险确实需要时才升级到更重的流程。
...
- [/tdd](./skills/tdd/SKILL.md) —— 测试驱动开发。作为纯 TDD 会话的独立参考；`/code-implement` 已为日常实现内嵌了最小 TDD 核心。该 skill 完全来源于 [Mattpocock: TDD](https://github.com/mattpocock/skills/blob/main/skills/engineering/tdd/SKILL.md)。
...
- [/clean-code](./skills/clean-code/SKILL.md) —— 基于《代码整洁之道》原则整理代码。作为专门清理的独立参考；`/code-implement` 已为日常实现内嵌了最小 Clean Code 核心。该 skill 当前以本仓库内的本地文档为准，完全来源于 [antigravity-awesome-skills](https://github.com/sickn33/antigravity-awesome-skills/blob/main/skills/clean-code/SKILL.md)。
